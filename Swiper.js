...
class Swiper extends Component {
    ...
    animateStack = () => {
        ...
        while (stackSize-- >= 0 && showSecondCard && !swipedAllCards) {
            ...
            if (
                this.state[`stackPositionY${stackSize}`] &&
                this.state[`stackPositionX${stackSize}`] &&
                ...
                const newWidth =
                    (100 -
                        (stackScale * 2 * (stackSize - 1) * 100) / cardWidth) *
                    0.01;
                const newHeight =
                    (100 +
                ...
                Animated.parallel([
                    ...
                    Animated.spring(this.state[`stackPositionX${stackSize}`], {
                        toValue: newSeparationX,
                        friction: this.props.stackAnimationFriction,
                        tension: this.props.stackAnimationTension,
                        useNativeDriver: true,
                    }),
                    Animated.spring(this.state[`stackWidth${stackSize}`], {
                        toValue: newWidth,
                        friction: this.props.stackAnimationFriction,
                        tension: this.props.stackAnimationTension,
                        useNativeDriver: true,
                    }),
                    Animated.spring(this.state[`stackHeight${stackSize}`], {
                        toValue: newHeight,
                        friction: this.props.stackAnimationFriction,
                        tension: this.props.stackAnimationTension,
                        useNativeDriver: true,
    ...
    rebuildStackValues = () => {
        ...
        for (let position = 0; position < stackSize; position++) {
            ...
            stackPositionsAndScales[`stackWidth${position}`] =
                new Animated.Value(
                    (100 - (stackScale * 2 * position * 100) / cardWidth) *
                        0.01,
                );
    ...
    calculateStackCardZoomStyle = position => [
        ...
        this.getCardStyle(),
        {
            zIndex: position * -1,
            transform: [
                { translateY: this.state[`stackPositionY${position}`] },
                { translateX: this.state[`stackPositionX${position}`] },
                { scaleX: this.state[`stackWidth${position}`] },
                { scaleY: this.state[`stackHeight${position}`] },
            ],
        },
        this.props.cardStyle,
    ];
